        ...
        tabData.data.map((el)=>(
          el.attributes.info?(
              <li className="flex m-0 justify-center items-center p-0 cursor-pointer relative h-16 desktop:hidden">
                <Link href='/' key={el.id} className=' inline-block h-16 my-0 mx-3 leading-lineheight text-sm before:absolute text-slate-500 before:inset-0 '>
                  {el.attributes.name}
                </Link>
            ...
            <NavLink  href='events/all' className={`before:absolute  before:inset-0  hover:border-b-2 hover:border-b-blue-500 hover:text-slate-500 focus:text-blue-500`}>{el.attributes.name}</NavLink>    
          )
        )
